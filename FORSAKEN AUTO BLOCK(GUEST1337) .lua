...
-- Guest变量
local guestAutoBlockAudioOn = true  -- 自动格挡音频开启
local guestAutoPunchOn = false  -- 自动出拳开启
local guestVerifyFacingCheckOn = true  -- 验证面向检查开启
local guestShowVisionRange = true  -- 显示视野范围
local guestDetectionRange = 9  -- 检测范围
local guestDetectionRangeSq = guestDetectionRange * guestDetectionRange  -- 检测范围平方
local guestVisionRange = 9  -- 视野范围
local guestVisionAngle = 150  -- 视野角度
local guestAimPunchActive = true  -- 出拳瞄准激活
local guestPunchPrediction = 1  -- 出拳预测
local guestAimPunchDuration = 0.8  -- 出拳瞄准持续时间
...
local guestTrackedPunchAnimations = {
    ...
    ["81905101227053"] = true, ["108807732150251"] = true,
}

-- 出拳瞄准的目标杀手用户名列表
local guestAimTargets = {"Slasher", "c00lkidd", "JohnDoe", "1x1x1x1", "Noli", "Sixer", "Nosferatu"}

local guestHumanoid, guestHRP = nil, nil  -- 本地角色的Humanoid和HumanoidRootPart
local guestPunchAiming = false  -- 是否正在出拳瞄准
local guestPunchLastTriggerTime = 0  -- 上次出拳触发时间
local guestOriginalWS, guestOriginalJP, guestOriginalAutoRotate = nil, nil, nil  -- 原始角色属性
local guestAimConnection = nil  -- 动画播放连接
local guestSoundHooks = {}  -- 声音钩子表
local guestSoundBlockedUntil = {}  -- 声音格挡冷却表
local guestLastLocalBlockTime = 0  -- 上次本地格挡时间
local guestLocalBlockCooldown = 0.7  -- 本地格挡冷却时间
local guestAutoPunchDelay = 0.5  -- 自动出拳延迟
local guestSoundBlockDuration = 0.4  -- 声音格挡持续时间
...
local function guestFireRemoteBlock()
    guestRemoteEvent:FireServer("UseActorAbility", { buffer.fromstring("\"Block\"") })
end

-- 触发远程出拳能力
local function guestFireRemotePunch()
    guestRemoteEvent:FireServer("UseActorAbility", { buffer.fromstring("\"Punch\"") })
end
...
local function guestAttemptBlock(char, hrp)
    ...
    local t = tick()
    if t < guestLastLocalBlockTime + guestLocalBlockCooldown then return end
...
-- 更新连接部分的大小和方向
local function guestUpdateConnectionPart(part, startPos, endPos)
...
local function guestUpdateVisionCone()
    if not guestShowVisionRange then
        if guestCenterPart then guestCenterPart.Position = Vector3.new(0, -1000, 0) end
        if guestLeftPart then guestLeftPart.Position = Vector3.new(0, -1000, 0) end
        if guestRightPart then guestRightPart.Position = Vector3.new(0, -1000, 0) end
        if guestLeftToKillerPart then guestLeftToKillerPart.Position = Vector3.new(0, -1000, 0) end
        if guestRightToKillerPart then guestRightToKillerPart.Position = Vector3.new(0, -1000, 0) end
        if guestLeftToCenterPart then guestLeftToCenterPart.Position = Vector3.new(0, -1000, 0) end
        if guestCenterToRightPart then guestCenterToRightPart.Position = Vector3.new(0, -1000, 0) end
        return
    end
    ...
    pcall(function()
        ...
        local centerPos = targetHRP.Position + look * guestVisionRange
        local halfAngle = math.rad(guestVisionAngle / 2)
        ...
        local leftPos = targetHRP.Position + leftDir * guestVisionRange
        local rightPos = targetHRP.Position + rightDir * guestVisionRange
        ...
        guestUpdateConnectionPart(guestLeftToKillerPart, leftPos, targetHRP.Position)
        guestUpdateConnectionPart(guestRightToKillerPart, rightPos, targetHRP.Position)
        guestUpdateConnectionPart(guestLeftToCenterPart, leftPos, centerPos)
        guestUpdateConnectionPart(guestCenterToRightPart, centerPos, rightPos)
    end)
end
